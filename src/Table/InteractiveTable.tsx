...
import type { TableProps as BaseTableProps } from "./BaseTable";

type SortProps = {
    headers?: (NonNullable<BaseTableProps["headers"]>[number] | TableProps.SortableColumn)[];
    onSort?: (column: number, order: TableProps.SortingOrder) => void;
    defaultSort?: TableProps.SortingState;
    sort?: TableProps.SortingState;
};

export type TableProps = Omit<BaseTableProps, "headers"> & SortProps;
...
export const Table = memo((props: TableProps) => {
    const { data, headers, onSort, defaultSort, sort, caption, ...rest } = props;
                ...
                <thead>
                    <tr>
                        {headers.map((header, i) => {
                            const label = isSortableColumn(header) ? header.label : header;
